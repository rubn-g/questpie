...
description: End-to-end tour of the barbershop booking app — collections, functions, jobs, admin, and frontend.
---
...
function BookingWizard() {
  const { data: barbers } = useQuery({
    queryKey: ["barbers"],
    queryFn: () => client.rpc.getActiveBarbers({}),
  });

  const booking = useMutation({
    mutationFn: (data) => client.rpc.createBooking(data),
    onSuccess: (result) => {
...
| Server functions (RPC) | `functions/create-booking.ts` |
