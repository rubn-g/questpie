...
|---|---|---|
| `factoryFunctions` + **named export**, single match | Filename (camelCase) | `blocks/hero.ts` → `hero` |
| `factoryFunctions` + **named exports**, multiple | Export name | Two exports → each export name |
| `factoryFunctions` + **default export**, has string arg | **Factory string arg** (kebab→camelCase) | `views/table.ts` with `export default view("collection-table", ...)` → `collectionTable` |
