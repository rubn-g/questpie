...
Services are reusable units of logic that get injected into `AppContext` under the `services` key. Define a service in `services/`, and it becomes available in every hook, function, job, and route handler.
...
```ts title="functions/get-post-stats.ts"
export default fn({
  schema: z.object({ postId: z.string() }),
  handler: async ({ input, collections, services }) => {
    const post = await collections.posts.findOne({ where: { id: input.postId } });
    return {
...
- [Functions](/docs/backend/business-logic/functions) — Use services in function handlers
