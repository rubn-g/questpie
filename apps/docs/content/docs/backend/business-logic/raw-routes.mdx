...
```ts
route().post().handler(async ({ request }) => new Response("OK"))
route().get().raw().handler(async ({ request }) => new Response("OK"))
route().patch().schema(z.object({ title: z.string() })).handler(async ({ input }) => ({ ok: true }))
```
...
- [Routes (JSON)](/docs/backend/business-logic/routes) — Typed JSON route handlers
