...
export type AppConfig = {
  collections: {
    posts: {
      select: { id: string; title: string; body: string | null; status: "draft" | "published" };
      insert: { title: string; body?: string | null; status?: "draft" | "published" };
      where: { title?: string | { contains?: string }; status?: "draft" | "published" | { in?: string[] } };
...
The client uses `AppConfig` which includes collections, globals, and functions:
...
// Autocomplete: rpc.createBooking, rpc.getActiveBarbers, ...
...
Function return types are inferred from the handler:
...
const barber = await client.rpc.getBarber({ barberId: "abc" });
...
- [Functions](/docs/backend/business-logic/functions) — Server functions
