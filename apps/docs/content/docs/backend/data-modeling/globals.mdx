...
| `.options({...})`                          | Schema, timestamps, versioning, workflow |
...
```ts
.options({
  schema: "web",     // optional Postgres schema (default: public)
  timestamps: true,
  versioning: true,
})
```

Globals can also use workflow stages through `versioning.workflow`. See [Versioning & Workflow](/docs/backend/data-modeling/versioning-workflow).

#### `schema`

Places the global's tables (main, i18n, versions, i18n_versions) under a named
Postgres schema instead of `public`. `migrate:generate` prepends
`CREATE SCHEMA IF NOT EXISTS "<name>";` when the schema is first introduced.
...
When unset, tables live in `public` — backward-compatible.
...
- [Versioning & Workflow](/docs/backend/data-modeling/versioning-workflow) — Version history and publishing stages
