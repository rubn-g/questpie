...
## The `#questpie/factories` Alias

The `#questpie/factories` import in collection/global files resolves to generated helpers that provide typed builders:
...
| ------------- | -------------- | ----------- | -------------------------------- |
| `collections` | `collections/` | `*.ts`      | Factory string arg → export name |
| `globals`     | `globals/`     | `*.ts`      | Factory string arg → export name |
| `routes`      | `routes/`      | `*.ts`      | Filename → camelCase/slash path  |
| `jobs`        | `jobs/`        | `*.ts`      | Filename → camelCase             |
| `services`    | `services/`    | `*.ts`      | Filename → camelCase             |
| `emails`      | `emails/`      | `*.ts`      | Filename → camelCase             |
| `seeds`       | `seeds/`       | `*.seed.ts` | Array (ordered)                  |
| `blocks`      | `blocks/`      | `*.ts`      | Factory string arg → export name |
...
| `adminPlugin`       | Discovers admin views, blocks, components, fields, and `config/admin.ts` |
