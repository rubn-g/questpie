...
function findMatchingBracket(str, start) {
  ...
  while (i < str.length) {
    const ch = str[i];
    if (inString) { if (ch === stringChar && str[i-1] !== '\\') inString = false; i++; continue; }
    if (ch === '"' || ch === "'" || ch === '`') { inString = true; stringChar = ch; i++; continue; }
    if (ch === '[') depth++;
    else if (ch === ']') { depth--; if (depth === 0) return i + 1; }
    i++;
...
function parseObjectLiteral(str) {
  ...
  while (i < str.length) {
    ...
    if (str[i] === '"' || str[i] === "'") {
      const q = str[i]; i++;
      while (i < str.length && str[i] !== q) { key += str[i]; i++; }
      i++;
    } else {
      while (i < str.length && /[a-zA-Z0-9_$]/.test(str[i])) { key += str[i]; i++; }
    }
    if (!key) break;
    ...
    if (str[i] === '{') {
      ...
      rawValue = str.slice(i, end); i = end;
    } else if (str[i] === '[') {
      ...
      if (afterArr) { rawValue += afterArr[0]; i += afterArr[0].length; }
    } else if (str[i] === '"' || str[i] === "'" || str[i] === '`') {
      const q = str[i]; i++; let s = '';
      ...
      if (str.slice(j, j+2) === '=>') {
        ...
        if (str[j] === '{') {
          ...
          // Expression body
          let exprEnd = j, depth = 0;
          let inStr = false, strCh = '';
          while (exprEnd < str.length) {
            const c = str[exprEnd];
            if (inStr) { if (c === strCh) inStr = false; exprEnd++; continue; }
            if (c === '"' || c === "'" || c === '`') { inStr = true; strCh = c; exprEnd++; continue; }
            if (c === '(' || c === '[' || c === '{') depth++;
      ...
      while (j < str.length) {
        const ch = str[j];
        if (inStr) { if (ch === strCh) inStr = false; j++; continue; }
        if (ch === '"' || ch === "'" || ch === '`') { inStr = true; strCh = ch; j++; continue; }
        if (ch === '(' || ch === '[' || ch === '{') depth++;
...
function buildChain(fieldType, pairs) {
  ...
  if (fieldType === 'text' || fieldType === 'email' || fieldType === 'url') {
    if (pairMap.has('maxLength')) { baseArg = pairMap.get('maxLength'); pairMap.delete('maxLength'); }
  } else if (fieldType === 'select') {
    if (pairMap.has('options')) { baseArg = pairMap.get('options'); pairMap.delete('options'); }
  } else if (fieldType === 'relation') {
    if (pairMap.has('to')) { baseArg = pairMap.get('to'); pairMap.delete('to'); }
  }
  ...
  if (isHasMany && pairMap.has('through')) {
    ...
    if (pairMap.has('sourceField')) { parts.push(`sourceField: ${pairMap.get('sourceField')}`); pairMap.delete('sourceField'); }
    if (pairMap.has('targetField')) { parts.push(`targetField: ${pairMap.get('targetField')}`); pairMap.delete('targetField'); }
    if (pairMap.has('relationName')) { parts.push(`relationName: ${pairMap.get('relationName')}`); pairMap.delete('relationName'); }
    chains.push(`.manyToMany({ ${parts.join(', ')} })`);
    pairMap.delete('hasMany'); pairMap.delete('through');
  } else if (isHasMany && pairMap.has('foreignKey')) {
    const parts = [`foreignKey: ${pairMap.get('foreignKey')}`];
    if (pairMap.has('onDelete')) { parts.push(`onDelete: ${pairMap.get('onDelete')}`); pairMap.delete('onDelete'); }
    if (pairMap.has('relationName')) { parts.push(`relationName: ${pairMap.get('relationName')}`); pairMap.delete('relationName'); }
    chains.push(`.hasMany({ ${parts.join(', ')} })`);
    pairMap.delete('hasMany'); pairMap.delete('foreignKey');
  } else if (isHasMany) {
  ...
  for (const [key, val] of pairMap) {
    switch (key) {
      case 'required': if (val === 'true') chains.push('.required()'); break;
      case 'default': chains.push(`.default(${val})`); break;
      case 'label': chains.push(`.label(${val})`); break;
      case 'description': chains.push(`.description(${val})`); break;
      case 'localized': if (val === 'true') chains.push('.localized()'); break;
      case 'virtual': if (val === 'true') chains.push('.virtual()'); else chains.push(`.virtual(${val})`); break;
      case 'input':
        if (val === '"optional"' || val === "'optional'") chains.push('.inputOptional()');
        else if (val === 'false') chains.push('.inputFalse()');
        break;
      case 'output': if (val === 'false') chains.push('.outputFalse()'); break;
      case 'min': chains.push(`.min(${val})`); break;
      case 'max': chains.push(`.max(${val})`); break;
      case 'access': chains.push(`.access(${val})`); break;
      case 'hooks': chains.push(`.hooks(${val})`); break;
      case 'onDelete': chains.push(`.onDelete(${val})`); break;
      case 'relationName': chains.push(`.relationName(${val})`); break;
      case 'multiple': if (val === 'true') chains.push('.multiple()'); break;
      case 'unique': if (val === 'true') chains.push('.unique()'); break;
      case 'meta':
...
function transformObject(pairs, fullMatch) {
  ...
  if (arrowMatch) {
    ...
    if (arrowMatch) {
      ...
      const parenContent = fieldsContent.replace(/^\(\)\s*=>\s*\(/, '').replace(/\)$/, '').trim();
...
function processFile(filepath) {
  ...
  const fieldTypes = [
    'object', 'array', // Process these first (they may contain inner V1 calls)
    'text', 'textarea', 'number', 'boolean', 'datetime', 'date', 'time',
    'json', 'richText', 'select', 'email', 'url', 'slug',
    'relation', 'upload',
  ];
  ...
  while (changed && iterations < 30) {
    ...
    for (const fieldType of fieldTypes) {
      ...
      while (true) {
        ...
        if (braceEnd === -1) { searchStart = idx + pattern.length; continue; }

        // Find closing paren
        let afterBrace = braceEnd;
        while (afterBrace < content.length && /\s/.test(content[afterBrace])) afterBrace++;
        if (content[afterBrace] !== ')') { searchStart = idx + pattern.length; continue; }
        ...
        if (transformed !== fullMatch) {
          content = content.slice(0, idx) + transformed + content.slice(fullEnd);
          changed = true;
